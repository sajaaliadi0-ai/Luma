...
function Overview({ dark }) {
  ...
  const isSuperAdmin =
    storedRole === "superadmin" || storedRole === "super_admin";
  ...
  useEffect(() => {
    const loadOverview = async () => {
      try {
        ...
        const usersResponse = await api.get("/api/superadmin/users");

        // -----------------------------------------------------
        // BLUEPRINTS
        // -----------------------------------------------------

        const blueprintsResponse = await api.get("/api/superadmin/blueprints");

        // -----------------------------------------------------
        // LOGS
        // -----------------------------------------------------

        const logsResponse = await api.get("/api/superadmin/logs");
        ...
        if (isSuperAdmin) {
          const statsResponse = await api.get("/api/superadmin/system-stats");
        ...
        <main className="overview-page">
          ...
          <header className="overview-header">
            <div className="overview-title-area">
              <span className="overview-eyebrow">
                {isSuperAdmin
                  ? t("overviewSuperAdminDashboard")
                  : t("overviewAdminDashboard")}
              </span>
            ...
            <div className="overview-profile">
              <div className="overview-avatar">{getInitial()}</div>

              <div className="overview-profile-info">
                <strong>
                  {isSuperAdmin ? t("overviewSuperAdmin") : t("overviewAdmin")}
                </strong>
            ...
            <div className="overview-banner-content">
              ...
              <div>
                <span className="overview-banner-label">
                  {t("overviewControlCenter")}
                </span>

                <h2>{t("overviewEverythingUnderControl")}</h2>

                <p>{t("overviewBannerDescription")}</p>
              </div>
            </div>
          ...
          <section className="overview-stats">
            ...
            <div className="overview-stat-card">
              <div className="overview-stat-top">
                ...
                <span className="overview-stat-badge">{t("overviewLive")}</span>
              </div>

              <div className="overview-stat-info">
                <span>{t("overviewTotalUsers")}</span>

                <strong>{loading ? "..." : users.length}</strong>

                <small>{t("overviewRegisteredUsers")}</small>
              </div>
            </div>
            ...
            <div className="overview-stat-card">
              ...
              <div className="overview-stat-info">
                <span>{t("overviewBlueprints")}</span>

                <strong>{loading ? "..." : blueprints.length}</strong>

                <small>{t("overviewGeneratedBlueprints")}</small>
              </div>
            </div>
            ...
            <div className="overview-stat-card">
              ...
              <div className="overview-stat-info">
                <span>{t("overviewActivities")}</span>

                <strong>{loading ? "..." : logs.length}</strong>

                <small>{t("overviewRecordedActivities")}</small>
              </div>
            </div>
            ...
            {isSuperAdmin && (
              <div className="overview-stat-card">
                ...
                <div className="overview-stat-info">
                  <span>{t("overviewSystemStatus")}</span>

                  <strong>{systemStats ? t("overviewOnline") : "..."}</strong>

                  <small>{t("overviewSystemMonitoring")}</small>
                </div>
              </div>
            )}
          </section>
          ...
          <section className="overview-grid">
            ...
            <div className="overview-card overview-system-card">
              <div className="overview-card-header">
                <div>
                  ...
                  <h3>{t("overviewSystemOverview")}</h3>

                  <p>{t("overviewCurrentPlatformInfo")}</p>
                </div>
              ...
              {isSuperAdmin ? (
                <div className="overview-system-panel">
                  <div className="overview-system-header">
                    <div className="system-online">
                      ...
                      {t("overviewSystemOperational")}
                    </div>

                    <span className="system-live">{t("overviewLive")}</span>
                  </div>
                  ...
                  {systemStats ? (
                    <div className="system-data-grid">
                      {Object.entries(systemStats)
                        .slice(0, 4)
                        .map(([key, value]) => (
                          <div className="system-data-item" key={key}>
                            <span>
                              {key
                                .replaceAll("_", " ")
                    ...
                    <div className="overview-no-data">
                      ...
                      <span>{t("overviewNoSystemStats")}</span>
                    </div>
                  )}
                </div>
              ) : (
                ...
                <div className="overview-blueprint-summary">
                  <div className="blueprint-summary-icon">✦</div>

                  <div>
                    <strong>{loading ? "..." : blueprints.length}</strong>

                    <span>{t("overviewTotalBlueprints")}</span>
                  </div>
            ...
            <div className="overview-card overview-activity-card">
              <div className="overview-card-header">
                <div>
                  <span className="overview-card-label">
                    {t("overviewMonitoring")}
                  </span>

                  <h3>{t("overviewRecentActivity")}</h3>

                  <p>{t("overviewLatestEvents")}</p>
                </div>
                    ...
                    >
                      ...
                      <div className="activity-content">
                        <strong>
                          {log.action ||
                            log.event ||
                            t("overviewPlatformActivity")}
                        </strong>

                        <p>
                          {log.user ||
                            log.user_name ||
                            t("overviewSystemActivity")}
                        </p>
          ...
          <section className="overview-ai-panel">
            <div className="overview-ai-left">
              ...
              <div>
                <span className="ai-label">{t("overviewAiCore")}</span>

                <h3>{t("overviewAssistantReady")}</h3>

                <p>{t("overviewAssistantDescription")}</p>
              </div>
            </div>
